...
public:
    ...
    int getNumFullSeats() const     { return numFullSeats; }
    ...
    void setDeparture(string dep)       { departure = std::move(dep); };
    /// @brief Set fight destination
    void setDestination(string dest)    { destination = std::move(dest); };
    /// @brief Set fight departure time
    void setDepartureTime(string dTime) { departureTime = std::move(dTime); };
    /// @brief Adds a passenger to a specific seat
    void addPassenger(int i, Person p)  { seats[i].setPerson(std::move(p)); numFullSeats++; }
