...
public:
    ...
    void setFName(string fN) { fName = std::move(fN); }

    /// @brief Set the person's last name
    void setLName(string lN) { lName = std::move(lN); }

    /// @brief Set the person's address
    void setAddress(string add) { address = std::move(add); }

    /// @brief Set the person's phone number
    void setPhoneNum(string num) { phoneNum = std::move(num); }
